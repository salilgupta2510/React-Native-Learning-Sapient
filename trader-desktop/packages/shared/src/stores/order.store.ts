...
export class OrderStore {
    ...
    createOrder = (jsOrder: JsOrder): Promise<any> => {
        ...
        // Creates one order
        return (<HttpOrderAdapter> orderAdapter).createOrder(jsOrder)
            .then(respone => {
                this.orderMap.set(id, new Order(id, side, symbol, quantity, committed, executed));
                return respone;
            });
    ...
    deleteAllOrders = () => {
        if(this.count) {
            return this.adapters.serverAdapter.reset()
            .then(() => {
                this.orderMap.clear();
            });
        } else {
    ...
    createOrdersAtServer = (numOrdersToCreate?: number): Promise<any> => {
        ...
        return (<HttpOrderAdapter> orderAdapter).createOrders(orders)
            .then(response => {
